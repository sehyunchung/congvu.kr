...
function Rig() {
  ...
  return useFrame((state) => {
    camera.position.z = state.clock.getElapsedTime() * -1;
  });
}
  ...
  return (
    <Canvas camera={{ position: [0, 0, 20], fov: 90 }}>
      <RemixBridge>
        ...
        <pointLight intensity={1.8} position={[0, 0, 3500]} />
        <Suspense fallback={null}>
          <Cloud position={[-24, -2, -35]} speed={0.2} opacity={0.3} />
          ...
          <Cloud position={[8, 2, -100]} speed={0.2} opacity={0.15} />
        </Suspense>
        <Sky
          azimuth={3}
          turbidity={1}
